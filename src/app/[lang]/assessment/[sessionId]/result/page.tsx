...
const RISK_CONFIG = {
  low:      { color: "#10B981", bg: "bg-emerald-50", text: "text-emerald-600", border: "border-emerald-200", label: "Low Risk",      icon: Activity },
...
export default async function ResultPage({ params }: { params: Promise<{ lang: string; sessionId: string }> }) {
  ...
  const [assessment] = await db.select().from(assessments)
    .where(and(eq(assessments.id, sessionId), eq(assessments.userId, userId))).limit(1);
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 sm:p-8 animate-up">
      ...
      <div className="text-center mb-10">
        <ClipboardCheck size={48} className="text-emerald-800 mx-auto mb-4" />
      ...
      <div className={`rounded-3xl border text-center p-8 mb-8 shadow-sm ${risk.bg} ${risk.border}`}>
        ...
        <div className="text-sm font-semibold text-slate-500 uppercase tracking-widest mb-4">{isBn ? "সামগ্রিক স্কোর" : "Overall Score"} / 100</div>
      ...
      {scores.length > 0 && (
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm mb-8">
          ...
          {scores.map(({ ts, t }) => {
            ...
            const scoreColorClass = pct >= 70 ? "bg-red-500 text-red-700" : pct >= 40 ? "bg-orange-500 text-orange-700" : "bg-emerald-500 text-emerald-700";
            const barColorClass = pct >= 70 ? "bg-red-500" : pct >= 40 ? "bg-orange-500" : "bg-emerald-500";
            return (
      ...
      {triggeredConditions.length > 0 && (
        <div className="mb-8">
          ...
          {triggeredConditions.map(({ ac, c }) => {
            ...
            return (
              <div key={ac.id} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm mb-4 relative overflow-hidden">
                ...
                <div className="flex flex-wrap justify-between items-start gap-4 mb-3">
                  <div>
                    ...
                    <div className="text-sm text-slate-500 font-medium">{isBn && c.laypersonNameBn ? c.laypersonNameBn : c.laypersonNameEn}</div>
                  </div>
                  <div className="text-right">
                    ...
                    <div className="text-xs text-slate-500 mt-1.5 font-medium">{c.urgencyLabel}</div>
                  </div>
                </div>
                <p className="text-sm text-slate-600 mb-4 leading-relaxed">{c.descriptionEn}</p>
                {c.nextStepsEn && (
                  <div className="text-sm p-4 bg-slate-50 border border-slate-100 rounded-xl whitespace-pre-line text-slate-700 leading-relaxed mb-4">
                ...
                {c.specialistType && (
                  <div className="text-sm text-slate-600 mb-3 flex items-center gap-1.5">
                    ...
                    {isBn ? "বিশেষজ্ঞ:" : "Specialist:"} <strong className="text-emerald-900">{c.specialistType}</strong>
                  </div>
                )}
                <div className="flex flex-wrap items-center justify-between gap-4 mt-4 pt-4 border-t border-slate-100">
                  <div className="text-xs text-slate-400 italic flex-1">{c.disclaimer}</div>
                  <div className="text-xs font-bold px-2.5 py-1 bg-emerald-50 text-emerald-700 rounded-lg border border-emerald-100">
                    {ac.probabilityLabel}
                  </div>
                </div>
              </div>
          ...
          <div className="flex flex-col gap-1">
            {answers.map(({ a, q, o }) => q && (
              <div key={a.id} className="flex justify-between gap-4 py-3 border-b border-slate-100 last:border-0 items-center">
                <span className="text-slate-600 text-sm font-medium">{q.text}</span>
                <span className="text-sm font-bold text-slate-900 text-right bg-slate-50 px-2 py-1 rounded">{o?.label ?? a.freeTextValue ?? `${a.numericValue}`}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-wrap justify-center gap-3 mt-10">
        <Link href={`/${lang}/clinicians`} className="bg-emerald-800 hover:bg-emerald-700 text-white font-semibold py-2.5 px-6 rounded-lg transition-colors shadow-sm flex items-center gap-2 text-sm">
          <MapPin size={18} /> {isBn ? "বিশেষজ্ঞ খুঁজুন" : "Find a Specialist"}
        </Link>
        <Link href={`/${lang}/assessment`} className="bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-semibold py-2.5 px-6 rounded-lg transition-colors shadow-sm flex items-center gap-2 text-sm">
          <RefreshCcw size={18} /> {isBn ? "আবার মূল্যায়ন করুন" : "Start Another Assessment"}
        </Link>
        <Link href={`/${lang}/dashboard`} className="bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-semibold py-2.5 px-6 rounded-lg transition-colors shadow-sm flex items-center gap-2 text-sm">
