    ...
    <div className="min-h-[calc(100vh-60px)] flex items-center justify-center p-8">
      <div className="w-full max-w-lg">
        <div className="text-center mb-8">
          ...
          <p className="text-stone-500 text-sm">{d.registerSubtitle}</p>
        </div>

        {/* Role toggle */}
        <div className="flex gap-2 mb-6 bg-stone-100 rounded-xl p-1.5 border border-stone-200">
          {(["user", "expert"] as const).map(r => (
            <button key={r} onClick={() => setRole(r)} className={`flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-lg border-none cursor-pointer text-sm font-semibold transition-all ${
              role === r ? "bg-orange-700 text-white shadow-sm" : "bg-transparent text-stone-500 hover:text-slate-700 hover:bg-slate-200/50"
            }`}>
        ...
        <form onSubmit={handleSubmit} className="bg-white/80 backdrop-blur-xl border border-stone-200 shadow-sm rounded-2xl p-6 sm:p-8 flex flex-col gap-5 animate-up">
          ...
          <div>
            ...
            <input id="reg-name" className="w-full px-4 py-2 bg-stone-50 border border-stone-200 rounded-lg text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Your full name" />
          ...
          <div className="flex flex-col sm:flex-row gap-5">
            <div className="flex-1">
              ...
              <input id="reg-age" className="w-full px-4 py-2 bg-stone-50 border border-stone-200 rounded-lg text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all" type="number" min={10} max={120} value={form.age} onChange={e => setForm(f => ({ ...f, age: e.target.value }))} placeholder="25" />
            </div>
            <div className="flex-1">
              <label className="block text-sm font-semibold text-slate-700 mb-1">{d.gender}</label>
              <select id="reg-gender" className="w-full px-4 py-2 bg-stone-50 border border-stone-200 rounded-lg text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all appearance-none" value={form.gender} onChange={e => setForm(f => ({ ...f, gender: e.target.value }))}>
          ...
          <div>
            ...
            <input id="reg-email" className="w-full px-4 py-2 bg-stone-50 border border-stone-200 rounded-lg text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all" type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="you@email.com" />
          ...
          <div>
            ...
            <input id="reg-password" className="w-full px-4 py-2 bg-stone-50 border border-stone-200 rounded-lg text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all" type="password" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} placeholder="••••••••" />
          ...
          <div>
            ...
            <input id="reg-confirm" className="w-full px-4 py-2 bg-stone-50 border border-stone-200 rounded-lg text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all" type="password" value={form.confirm} onChange={e => setForm(f => ({ ...f, confirm: e.target.value }))} placeholder="••••••••" />
            {errors.confirm && <div className="text-red-600 text-xs mt-1 font-medium">{errors.confirm}</div>}
          </div>

          {role === "expert" && (
            <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 text-sm text-orange-700 flex gap-2 items-start mt-2">
              <Stethoscope className="shrink-0 mt-0.5" size={16} />
              <p className="m-0 leading-relaxed font-medium">{d.expertNote}</p>
            </div>
          )}

          <button id="reg-submit" className="w-full bg-orange-700 hover:bg-orange-600 text-white font-semibold py-2.5 rounded-lg transition-colors mt-2" type="submit" disabled={loading}>
            {loading ? dict.common.loading : d.register}
          </button>

          <p className="text-sm text-stone-500 text-center mt-2">
            {d.hasAccount}{" "}
            <Link href={`/${lang}/auth/login`} className="text-orange-700 font-bold hover:underline">{d.signIn}</Link>
          </p>
        </form>
      </div>
    </div>
