...
function OptionRow({ opt, onChange, onDelete }: { opt: AnswerOption; onChange: (o: AnswerOption) => void; onDelete: () => void }) {
  ...
  const severityColor = isHighRisk ? "text-red-600" : isMedRisk ? "text-orange-500" : "text-emerald-600";
  const severityAccent = isHighRisk ? "accent-red-600" : isMedRisk ? "accent-orange-500" : "accent-emerald-600";

  return (
    <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center bg-slate-50 border border-slate-200 rounded-xl p-3">
      <input 
        className="flex-2 min-w-[120px] w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-900 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all font-medium" 
        value={opt.label} 
        placeholder="Option label..." 
        onChange={e => onChange({ ...opt, label: e.target.value })} 
      />
      <input 
        className="w-full sm:w-[120px] px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-900 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all font-medium font-mono" 
        value={opt.value} 
        placeholder="value-key" 
        onChange={e => onChange({ ...opt, value: e.target.value })} 
      />
      
      <div className="flex flex-col gap-1 w-full sm:w-[160px] shrink-0 bg-white p-2 rounded-lg border border-slate-200">
        <div className="flex justify-between items-center text-[0.7rem] uppercase tracking-wider font-bold">
          <span className="text-slate-400">Sev: {opt.severity}</span>
...
function QuestionEditor({ q, index, onChange, onDelete }: { q: Question; index: number; onChange: (q: Question) => void; onDelete: () => void }) {
  ...
  return (
    <div className="bg-white border-l-4 border-emerald-600 border-y border-r border-slate-200 rounded-r-2xl rounded-l-md p-6 mb-6 shadow-sm animate-up">
      <div className="flex justify-between items-start mb-4">
        <span className="bg-emerald-50 text-emerald-800 font-bold text-xs px-3 py-1 rounded-full border border-emerald-200 uppercase tracking-wide">Question {index + 1}</span>
        <button className="text-slate-400 hover:text-red-600 transition-colors flex items-center gap-1.5 text-sm font-semibold" onClick={onDelete}>
          <Trash2 size={16} /> Delete
        </button>
      </div>

      <div className="mb-5">
        <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Question Text</label>
        <textarea 
          className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all font-medium resize-y min-h-[80px]" 
          value={q.text} 
      ...
      <div className="mb-5">
        <div className="flex justify-between items-center mb-2">
          <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide m-0">Answer Options</label>
          <button className="flex items-center gap-1 text-emerald-700 bg-emerald-50 hover:bg-emerald-100 font-semibold px-3 py-1.5 rounded-lg text-xs transition-colors border border-emerald-200" onClick={addOption}>
        ...
        <div className="flex flex-col gap-2.5">
          ...
          {q.options.length === 0 && (
            <div className="text-center p-4 border-2 border-dashed border-slate-200 rounded-xl text-slate-400 font-medium text-sm">
      ...
      <label className="flex items-center gap-2 cursor-pointer text-sm font-medium text-slate-600 select-none bg-slate-50 w-fit px-3 py-2 rounded-lg border border-slate-200">
        <input 
          type="checkbox" 
          checked={q.required} 
  ...
  return (
    <div className="animate-up max-w-4xl mx-auto pb-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-6">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <Link href={`/${lang}/expert/dashboard/topics`} className="text-slate-500 hover:text-slate-800 font-semibold text-sm flex items-center transition-colors">
        ...
        <div className="flex gap-3">
          <button className="bg-white border border-slate-200 hover:border-emerald-500 text-slate-700 hover:text-emerald-800 font-semibold py-2 px-4 rounded-xl transition-colors shadow-sm flex items-center gap-2" onClick={addQuestion}>
      ...
      {isCustom && (
        <div className="flex gap-2 mb-8 bg-slate-100 p-1.5 rounded-2xl w-fit border border-slate-200">
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2 rounded-xl font-bold text-sm transition-all ${activeTab === "questions" ? "bg-white text-emerald-800 shadow-sm border border-slate-200/50" : "text-slate-500 hover:text-slate-700 hover:bg-slate-200/50"}`}
          >
            <HelpCircle size={16} /> Questions
          </button>
          <button
            onClick={() => setActiveTab("details")}
            className={`flex items-center gap-2 px-5 py-2 rounded-xl font-bold text-sm transition-all ${activeTab === "details" ? "bg-white text-emerald-800 shadow-sm border border-slate-200/50" : "text-slate-500 hover:text-slate-700 hover:bg-slate-200/50"}`}
          >
        ...
        <div className="animate-up">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm mb-6">
            <h4 className="text-lg font-bold text-slate-900 mb-6 flex items-center gap-2">
              <Settings className="text-emerald-800" size={20} /> Topic Metadata
            </h4>
                ...
                >
                  <span className="flex items-center gap-2">
                    {(() => {
                      ...
                      return isLucide ? <IconComp size={20} className="text-emerald-800" /> : <span className="text-xl">{metaIcon}</span>;
                    })()}
                    <span className="text-sm font-medium text-slate-600 truncate max-w-[100px]">{metaIcon}</span>
                  </span>
                    ...
                    {ICON_SUGGESTIONS.map(iconName => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center justify-center p-2 rounded-lg transition-all ${metaIcon === iconName ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-900 border border-transparent'}`}
                          title={iconName}
                        >
                          <IconComp size={20} />
                        </button>
            ...
            <div className="p-4 rounded-2xl flex items-center gap-4 transition-colors" style={{ backgroundColor: `${metaColor}10`, borderColor: `${metaColor}30`, borderWidth: 1 }}>
              ...
              <div>
                ...
                <div className="text-xs text-slate-500 font-medium">{metaDesc || "Topic description"}</div>
              </div>
            </div>
          </div>

          <div className="flex justify-end">
            <button className={`font-semibold py-2.5 px-6 rounded-xl transition-all shadow-sm flex items-center gap-2 ${saved ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' : 'bg-emerald-800 hover:bg-emerald-700 text-white'}`} onClick={save}>
      ...
      {(!isCustom || activeTab === "questions") && (
        <div className="animate-up">
          {questions.length === 0 && (
            <div className="bg-white border border-slate-200 rounded-3xl p-12 text-center shadow-sm mb-6">
              ...
              <p className="text-slate-500 font-medium mb-6">Add your first diagnostic question for this topic.</p>
              <button className="bg-emerald-800 hover:bg-emerald-700 text-white font-semibold py-2.5 px-6 rounded-xl transition-colors shadow-sm inline-flex items-center gap-2" onClick={addQuestion}>
          ...
          {questions.length > 0 && (
            <div className="flex flex-col sm:flex-row gap-3 mt-6">
              <button className="flex-1 bg-white border-2 border-dashed border-slate-300 hover:border-emerald-500 hover:bg-emerald-50 text-slate-600 hover:text-emerald-800 font-bold py-4 px-4 rounded-2xl transition-all flex items-center justify-center gap-2" onClick={addQuestion}>
