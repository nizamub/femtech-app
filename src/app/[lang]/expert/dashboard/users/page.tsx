      ...
      <div className="mb-8 flex justify-between items-center flex-wrap gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 mb-2 flex items-center gap-2">
            <Users className="text-orange-700" size={28} />
            Patient Monitoring
          </h2>
          <p className="text-sm text-stone-500 font-medium">{total} registered patients</p>
        </div>
      </div>
            ...
            <thead>
              <tr className="border-b border-slate-100">
                {["Patient", "Age / Gender", "Assessments", "Avg Score", "Latest Risk", "Last Active", ""].map(h => (
                  <th key={h} className="py-3 px-4 text-left text-xs font-bold text-stone-400 uppercase tracking-wider whitespace-nowrap">{h}</th>
                ))}
              </tr>
            </thead>
              ...
              {allUsers.map(u => {
                ...
                return (
                  <tr key={u.id} className="hover:bg-stone-50/50 transition-colors">
                    ...
                    <td className="py-4 px-4 text-sm font-medium text-stone-500">
                      {u.latestDate ? new Date(u.latestDate).toLocaleDateString() : "Never"}
                    </td>
                    <td className="py-4 px-4 text-right">
                      <Link href={`/${lang}/expert/dashboard/users/${u.id}`} className="inline-flex items-center gap-1 bg-orange-50 hover:bg-orange-100 text-orange-700 font-semibold py-1.5 px-3 rounded-lg transition-colors text-sm border border-orange-200">
          ...
          <div className="flex gap-2 justify-center mt-6">
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
              <Link key={p} href={`/${lang}/expert/dashboard/users?page=${p}`}
                className={`w-8 h-8 flex items-center justify-center rounded-lg font-semibold text-sm transition-colors border ${p === pageNum ? "bg-orange-700 border-emerald-800 text-white shadow-sm" : "bg-white border-stone-200 text-stone-600 hover:bg-orange-50 hover:text-orange-700"}`}>
                {p}
              </Link>
            ))}
          </div>
