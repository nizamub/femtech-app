...
const RISK_CONFIG: Record<string, { color: string; bg: string }> = { 
  low: { color: "text-orange-500", bg: "bg-orange-50" }, 
...
export default async function PatientProfilePage({ params }: { params: Promise<{ lang: string; userId: string }> }) {
  ...
  return (
    <div className="animate-up max-w-5xl mx-auto">
      <Link href={`/${lang}/expert/dashboard/users`} className="inline-flex items-center gap-1 text-sm font-semibold text-stone-500 hover:text-orange-700 transition-colors mb-6">
        <ArrowLeft size={16} /> Back to Patient Monitoring
      </Link>

      {/* Patient Header */}
      <div className="bg-white border border-stone-200 rounded-3xl p-6 shadow-sm mb-6 flex items-center gap-6 flex-wrap">
        <div className="w-16 h-16 rounded-full bg-orange-100 text-orange-700 flex items-center justify-center text-3xl font-bold shrink-0 shadow-sm border border-orange-200">
          {patient.name[0].toUpperCase()}
        </div>
        <div className="flex-1 min-w-0">
          <h2 className="text-2xl font-bold text-slate-900 mb-1 truncate">{patient.name}</h2>
          <div className="text-stone-500 text-sm font-medium mb-3 truncate">{patient.email}</div>
          <div className="flex gap-2 flex-wrap">
            ...
            <span className="text-xs px-2.5 py-1 bg-stone-100 text-stone-600 font-bold rounded-full border border-stone-200">
        ...
        {patientAssessments[0] && (() => {
          ...
          return (
            <div className="text-center bg-stone-50 p-4 rounded-2xl border border-slate-100 min-w-[140px]">
              <div className={`text-4xl font-black ${rConfig.color} leading-none mb-1`}>
                {Math.round(patientAssessments[0].overallScore ?? 0)}
              </div>
              <div className="text-[0.65rem] font-bold text-stone-400 uppercase tracking-wider mb-1">Latest Score</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        {latestScores.length > 0 && (
          <div className="bg-white border border-stone-200 rounded-2xl p-6 shadow-sm">
            <h3 className="font-bold text-slate-900 mb-5 flex items-center gap-2"><BarChart2 className="text-orange-700" size={20} /> Latest Topic Scores</h3>
            <div className="space-y-4">
              {latestScores.map(({ ts, t }) => {
                if (!t) return null;
                const pct = Math.round(ts.score);
                const scoreColorClass = pct >= 70 ? "text-red-600 bg-red-500" : pct >= 40 ? "text-orange-500 bg-orange-500" : "text-orange-500 bg-orange-500";
                  ...
                  <div key={ts.id}>
                    <div className="flex justify-between items-center mb-1.5 text-sm">
                      <span className="font-semibold text-slate-700 flex items-center gap-1.5">
                        <span className="text-orange-700 opacity-80">{isLucide ? <IconComp size={16} /> : <span>{t.icon}</span>}</span> {t.label}
                      </span>
        ...
        {latestConditions.length > 0 && (
          <div className="bg-white border border-stone-200 rounded-2xl p-6 shadow-sm">
            <h3 className="font-bold text-slate-900 mb-5 flex items-center gap-2"><Stethoscope className="text-orange-700" size={20} /> Conditions (Latest)</h3>
              ...
              {latestConditions.map(({ ac, c }) => {
                ...
                return (
                  <div key={ac.id} className={`p-4 rounded-xl border relative overflow-hidden bg-white ${rConfig.bg.replace('bg-', 'border-')}`}>
                    ...
                    <div className="text-xs text-stone-500 font-medium">{ac.probabilityLabel} · {c.urgencyLabel}</div>
                  </div>
                );
              })}
            </div>
      ...
      {patientAssessments.length > 0 && (
        <div className="bg-white border border-stone-200 rounded-2xl p-6 shadow-sm mb-6 overflow-hidden">
          <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2"><ClipboardList className="text-orange-700" size={20} /> Assessment History</h3>
          <div className="overflow-x-auto -mx-6 px-6">
            <table className="w-full border-collapse min-w-[500px]">
              <thead>
                <tr className="border-b border-slate-100">
                  {["Date", "Score", "Risk", "View"].map(h => (
                    <th key={h} className="py-3 px-4 text-left text-xs font-bold text-stone-400 uppercase tracking-wider">{h}</th>
                  ))}
                </tr>
              </thead>
                ...
                {patientAssessments.map(a => {
                  ...
                  return (
                    <tr key={a.id} className="hover:bg-stone-50/50 transition-colors">
                      ...
                      <td className="py-4 px-4">
                        <Link href={`/${lang}/assessment/${a.id}/result`} className="text-orange-500 hover:text-orange-700 font-semibold text-sm bg-orange-50 hover:bg-orange-100 px-3 py-1.5 rounded-lg transition-colors inline-block" target="_blank">
                          View →
                        </Link>
                      </td>
                    </tr>
      ...
      {/* Expert notes */}
      <div className="bg-white border border-stone-200 rounded-2xl p-6 shadow-sm">
        <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2"><FileText className="text-orange-700" size={20} /> Clinical Notes</h3>
        <form action={async (fd: FormData) => {
          ...
          <textarea name="note" className="w-full p-4 bg-stone-50 border border-stone-200 rounded-xl text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all text-sm resize-y min-h-[100px] mb-3" placeholder="Add a clinical note for this patient..." />
          <button type="submit" className="bg-slate-800 hover:bg-stone-900 text-white font-semibold py-2 px-5 rounded-xl transition-colors shadow-sm text-sm">Add Note</button>
        </form>

        {notes.length > 0 && (
          <div className="space-y-3">
            {notes.map(n => (
              <div key={n.id} className="p-4 bg-stone-50 rounded-xl border border-slate-100">
                <div className="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">{n.note}</div>
                <div className="text-[0.7rem] font-medium text-stone-400 mt-2">{new Date(n.createdAt).toLocaleString()}</div>
              </div>
            ))}
          </div>
        )}
