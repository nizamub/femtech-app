...
export default async function ExpertDashboardPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return (
    <div className="animate-up max-w-6xl mx-auto">
      ...
      <p className="text-sm text-stone-500 mb-8 font-medium">Manage diagnostic content, questions, and scoring settings.</p>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          { label: "Total Topics", value: allTopics.length, icon: Folder, colorClass: "text-orange-700" },
        ...
        {topicsWithStats.map(t => {
          ...
          return (
            <Link key={t.id} href={`/${lang}/expert/dashboard/topics/${t.id}`} 
              className={`bg-white border rounded-2xl p-4 shadow-sm transition-all hover:shadow-md ${t.visible ? 'border-stone-200' : 'border-stone-200 opacity-50 bg-stone-50'}`}
            >
              <div className="flex justify-between items-start mb-3">
                <span className="text-orange-700" style={{ color: color }}>
                  {isLucide ? <IconComp size={24} /> : <span className="text-2xl">{iconString}</span>}
                </span>
                <span className={`text-[0.65rem] font-bold px-2 py-0.5 rounded-full border ${t.visible ? "bg-orange-50 text-orange-600 border-orange-200" : "bg-stone-100 text-stone-500 border-stone-200"}`}>
                  {t.visible ? "Visible" : "Hidden"}
                </span>
              </div>
              <div className="font-bold text-slate-900 text-sm mb-1 line-clamp-1">{t.label}</div>
              <div className="text-xs text-stone-500 font-medium">{t.questionCount} questions</div>
            </Link>
          );
        })}
      </div>
