...
const RISK_CONFIG: Record<string, { color: string; bg: string; textHex: string; hexBg: string }> = { 
  low: { color: "text-orange-500", bg: "bg-orange-50", textHex: "#10B981", hexBg: "#10B98120" }, 
    ...
    <div className="animate-up max-w-6xl mx-auto">
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-slate-900 mb-2 flex items-center gap-2">
          <BarChart2 className="text-orange-700" size={28} />
          {d.analytics}
        </h2>
        <p className="text-sm text-stone-500 font-medium">Aggregate health data across all users and assessments.</p>
      </div>

      {/* KPI Cards */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          { label: d.totalUsers, value: totalUsersRow?.count ?? 0, icon: Users, colorClass: "text-orange-700" },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white border border-stone-200 rounded-2xl p-6 shadow-sm flex flex-col">
          <h3 className="font-bold text-slate-900 mb-5 flex items-center gap-2">
            <Activity className="text-orange-700" size={20} /> {d.riskDistribution}
          </h3>
              ...
              return (
                <div key={level} className="w-full">
                  <div className="flex justify-between items-center mb-1.5 text-sm">
                    ...
                    <span className="text-stone-500 font-semibold text-xs">{cnt} ({pct}%)</span>
                  </div>
        ...
        <div className="bg-white border border-stone-200 rounded-2xl p-6 shadow-sm">
          <h3 className="font-bold text-slate-900 mb-5 flex items-center gap-2">
            <Folder className="text-orange-700" size={20} /> Per-Topic Average Scores
          </h3>
          <div className="flex flex-col gap-3.5">
            {topicAvgs.map(t => {
              const score = Math.round(Number(t.avgScore ?? 0));
              const scoreColorClass = score >= 60 ? "text-red-600 bg-red-500" : score >= 35 ? "text-orange-500 bg-orange-500" : "text-orange-500 bg-orange-500";
              ...
              return (
                <div key={String(t.topicId)} className="flex items-center gap-3">
                  <div className="flex items-center gap-2 w-36 shrink-0">
                    <span className="text-orange-700 opacity-80">{isLucide ? <IconComp size={16} /> : <span>{t.icon}</span>}</span>
      ...
      <div className="bg-white border border-stone-200 rounded-2xl p-6 shadow-sm overflow-hidden">
        <div className="flex justify-between items-center mb-6">
          <h3 className="font-bold text-slate-900 m-0 flex items-center gap-2">
            <Clock className="text-orange-700" size={20} /> Recent Assessments
          </h3>
          <Link href={`/${lang}/expert/dashboard/users`} className="text-orange-700 hover:text-orange-900 bg-orange-50 hover:bg-orange-100 font-semibold py-1.5 px-4 rounded-xl transition-colors text-sm border border-orange-200">
            View All Users →
          </Link>
        </div>
                ...
                return (
                  <tr key={a.id} className="hover:bg-stone-50/50 transition-colors">
                    <td className="py-3 px-4">
                      ...
                      <div className="text-xs text-stone-500 font-medium">{a.userEmail}</div>
                    </td>
